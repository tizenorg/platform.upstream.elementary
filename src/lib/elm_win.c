...
_elm_win_state_change(Ecore_Evas *ee)
{
   ...
   const char *profile;
   Elm_Win_Conformant_Property property = ELM_WIN_CONFORMANT_DEFAULT;
   ...
   if (sd->indmode != (Elm_Win_Indicator_Mode)ecore_wl_window_indicator_state_get(sd->wl.win))
     {
        ...
        property |= ELM_WIN_CONFORMANT_INDICATOR_STATE;
     }
   if (sd->kbdmode != (Elm_Win_Keyboard_Mode)ecore_wl_window_keyboard_state_get(sd->wl.win))
     {
        ...
        ch_conformant = EINA_TRUE;
        property |= ELM_WIN_CONFORMANT_KEYBOARD_STATE;
     }
        ...
        if ((sd->ind.x != x) || (sd->ind.y != y) || (sd->ind.w != w) || (sd->ind.h != h))
          {
             ...
             sd->ind.h = h;
             ch_conformant  = EINA_TRUE;
             property |= ELM_WIN_CONFORMANT_INDICATOR_GEOMETRY;
          }
     }
        ...
        if ((sd->kbd.x != x) || (sd->kbd.y != y) || (sd->kbd.w != w) || (sd->kbd.h != h))
          {
             ...
             sd->kbd.h = h;
             ch_conformant  = EINA_TRUE;
             property |= ELM_WIN_CONFORMANT_KEYBOARD_GEOMETRY;
          }
     }
#endif
