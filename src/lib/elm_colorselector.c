...
_elm_colorselector_elm_widget_theme_apply(Eo *obj, Elm_Colorselector_Data *sd)
{
   ...
   if ((sd->mode == ELM_COLORSELECTOR_COMPONENTS) ||
       ...
       (sd->mode == ELM_COLORSELECTOR_BOTH))
     {
        if (!sd->col_bars_area) return THEME_APPLY_FAILED;
...
// TIZEN_ONLY(20160517): "elm,state,selected" signal is handled by
//                        _on_color_selected()
static void
...
_elm_color_item_eo_base_constructor(Eo *eo_item, Elm_Color_Item_Data *item)
{
   ...
   // TIZEN_ONLY(20160517): "elm,state,selected" signal is handled by
   //                        _on_color_selected()
   eo_do(VIEW(item), elm_obj_layout_signal_callback_add("elm,state,selected", "elm", _on_color_selected, item));
   //
