...
_elm_popup_elm_widget_theme_apply(Eo *obj, Elm_Popup_Data *sd)
{
   ...
   if (sd->action_area)
     {
        ...
        if (!elm_layout_theme_set(sd->action_area, "popup", buf, style))
          CRI("Failed to set layout!");
         */
        snprintf(buf, sizeof(buf), "buttons%i%s", sd->last_button_number, (sd->style_postfix.g ? "/popup" : ""));
   ...
   if (!elm_layout_theme_set(sd->content_area, "popup", "content", style))
     CRI("Failed to set layout!");
    */
   if (!elm_layout_theme_set(sd->content_area, "popup", (sd->style_postfix.g ? "content/popup" : "content"), elm_widget_style_get(obj)))
     CRI("Failed to set layout!");
   /* END */
   if (sd->text_content_obj)
...
_elm_popup_evas_object_smart_add(Eo *obj, Elm_Popup_Data *priv)
{
    ...
    */
     {
        Theme_Apply ret;
        const char *obj_style = elm_widget_style_get(obj);
        priv->style_postfix.g = EINA_FALSE;
        ...
        if (ret != THEME_APPLY_SUCCESS)
          {
             ...
             priv->style_postfix.g = EINA_TRUE;
          }
     }
    ...
    */
   if (!elm_layout_theme_set(priv->content_area, "popup", (priv->style_postfix.g ? "content/popup" : "content"), elm_widget_style_get(obj)))
     CRI("Failed to set layout!");
   else
